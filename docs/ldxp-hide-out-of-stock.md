...
为 `https://pay.ldxp.cn/shop/*` 商铺页面提供一个油猴脚本，强制显示百件以下的准确库存并自动隐藏库存小于或等于零的商品，只改变浏览器中的页面显示，不改变服务端库存及下单逻辑。
...
- `show_stock_type` 控制库存文案；`0 < stock_count < 100` 时，脚本在浏览器中将其强制改为 `1`，交由页面显示准确库存
- 商品卡片：`.goods_item`、`.goods-group-item` 或 `.goods-item`
- 缺货标记：商品卡片内的 `.stock.rank0`
- 页面已根据 `stock_count <= 0` 生成 `rank0`，脚本直接使用此渲染结果

Masonry 页面共有 20 张 `.goods_item` 卡片，其中 16 张缺货；分组页面共有 2 张 `.goods-group-item` 卡片，均为缺货；列表页面共有 56 张 `.goods-item` 卡片，其中 44 张缺货。

## 3. 功能要求

1. 脚本仅匹配 `https://pay.ldxp.cn/shop/*`。
2. 脚本须拦截 `goodsList` 的 XHR 响应，在页面处理响应前，将满足 `0 < extend.stock_count < 100` 的商品的 `extend.show_stock_type` 改为 `1`；其他商品保持接口原值。
3. 脚本须在 `document-start` 阶段注入以下 CSS 规则：
   ...
   html:not(.houmao-show-out-of-stock) :is(.goods_item, .goods-group-item, .goods-item):has(.stock.rank0) {
     display: none !important;
   }
   ```

4. 分类切换、搜索及“加载更多”插入的新商品须自动受同一规则约束。
5. Masonry 主题须改用原生 flex 重排剩余卡片，避免隐藏绝对定位卡片后留下空洞。
6. 不带 `.stock.rank0` 的商品卡片保持原样。
7. 有缺货商品时，在商品容器外层列表尾部增加“显示所有缺货商品”按钮，避免按钮参与商品布局。
8. 按钮须始终位于商品卡片之后；“加载更多”新增卡片后仍须归位到末尾。
9. 点击按钮后，本页展示全部缺货商品并移除所有该按钮；后续新增商品亦保持展示。
10. 刷新页面后恢复默认隐藏状态。
...
- 不在商品详情页工作。
- 不提供再次隐藏、持久化设置或缺货计数。
- 不额外请求接口，不改变服务端库存，不拦截下单，不自动刷新库存。
...
6. 非商铺页面及页面原有交互不受影响。
7. 三类下载页面离线检查应分别匹配 20 张卡片中的 16 张缺货卡片、2 张卡片中的 2 张缺货卡片，以及 56 张卡片中的 44 张缺货卡片。
8. 隐藏后剩余卡片须重新排列，不保留原绝对定位空洞。
9. 商品容器尾部显示“显示所有缺货商品”按钮。
10. 新卡片插入按钮之后时，按钮须自动移回末尾。
11. 点击按钮后，16 张缺货卡片全部显示，按钮消失；后续 DOM 更新不得重新创建按钮。
12. 接口返回 `show_stock_type: 0` 时，库存为 `1` 至 `99` 的商品须改为 `show_stock_type: 1`，库存为 `0`、`100` 或更大的商品须保持原值。
